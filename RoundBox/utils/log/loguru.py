...
DEFAULT_LOGURU: Dict = {
    "handlers": [
        ...
        # },
        {
            ...
            # "filter": only_info(),
            "level": logging.INFO,
            "format": "{time:YYYY-MM-DDTHH:mm:ss.SSS!UTC}Z <lvl>{level: <8}</lvl>{name}:{function}:{line}: <lvl>{message}</lvl>",
        },
        {
            "sink": "log/loguru.log",
            "enqueue": True,
...
class Logger:
    ...
    """
    def __init__(self, loguru_settings=None, level='DEBUG'):
        """

        :param loguru_settings:
        :param level:
        """

        if loguru_settings is None:
            self.loguru_settings = settings.LOGURU
        ...
        self.config = dict(chain(DEFAULT_LOGURU.items(), self.loguru_settings.items()))
